...
uint8 LSM303WriteReg(uint8 reg, uint8 dat)
{
  ...
  (((_32type*)(&(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00))->ODR)))->b15) = 0;
  spi_mosi(((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00)), buff, buff, 2); 
  (((_32type*)(&(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00))->ODR)))->b15) = 1;
...
char LSM303D_Init()
{
  ...
  if(0x49!=LSM303ReadReg(0x0F))
    return 1;


   
  LSM303WriteReg(0x20,(8<<4)             
                        |7<<0);                 
  LSM303WriteReg(0x21,(0<<6)       
                 |(3<<3));              



   
  LSM303WriteReg(0x24,4<<2);           
  LSM303WriteReg(0x25,0<<6);           
  LSM303WriteReg(0x26,0<<0);         
...
void LSM303A_Init( void )
{
  	...
  	u8 LSM303A_Init_Data[2] = {
			...
			0x10	 
                ...
                LSM303WriteReg(((u8)0x20),LSM303A_Init_Data[0]);
                LSM303WriteReg(((u8)0x23),LSM303A_Init_Data[1]);
...
void LSM303M_Init( void )
{
  	...
  	u8 LSM303M_Init_Data[2] = {
        ...
        LSM303WriteReg(((u8)0x01),LSM303M_Init_Data[0]);
        LSM303WriteReg(((u8)0x02),LSM303M_Init_Data[1]);
...
void LSM303M_Raed(short *pcompass)
{
  ...
  compassbuff[0] = LSM303ReadReg(0x08);
  compassbuff[1] = LSM303ReadReg(0x09);
  pcompass[0] = ((compassbuff[1]<<8)|compassbuff[0]);
  compassbuff[0] = LSM303ReadReg(0x0A);
  compassbuff[1] = LSM303ReadReg(0x0B);
  pcompass[1] = ((compassbuff[1]<<8)|compassbuff[0]);
  compassbuff[0] = LSM303ReadReg(0x0C);
  compassbuff[1] = LSM303ReadReg(0x0D);
...
void LSM303A_Raed(short *pacc)
{
  ...
  uint8 compassbuff[2] = {0};
  compassbuff[0] = LSM303ReadReg(((u8)0x28));
  compassbuff[1] = LSM303ReadReg(((u8)0x29));
  pacc[0]  = ((compassbuff[1]<<8)|compassbuff[0]);
  compassbuff[0] = LSM303ReadReg(((u8)0x2A));
  compassbuff[1] = LSM303ReadReg(((u8)0x2B));
  pacc[1] = ((compassbuff[1]<<8)|compassbuff[0]);
  compassbuff[0] = LSM303ReadReg(((u8)0x2C));
  compassbuff[1] = LSM303ReadReg(((u8)0x2D));
  ...
  float fTiltedX,fTiltedY = 0.0f;
	float fcosf=0;
      ...
      fcosf=fTiltedX /sqrt(fTiltedX*fTiltedX+fTiltedY*fTiltedY);
			...
			if(fTiltedY>0)
			  HeadingValue = (float)(acos(fcosf)*57.29577f);
			else
				HeadingValue =360-(float)(acos(fcosf)*57.29577f);
