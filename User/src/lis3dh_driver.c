...
AxesRaw_t LIS3DH_Acc_ADC = {0,0,0};
...
//  
  ...
  buff[1] = 0;          //先发送寄存器
  ACCEL_MAG_CS_OUT = 0;
  spi_mosi(LIS3DH_SPI, buff, buff, 2); //发送buff数据，并保存到buff里
  ACCEL_MAG_CS_OUT = 1;
  *Data = buff[1];                         //提取第二个数据
  /*返回状态寄存器的值*/
  return buff[1];
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data) {
  ...
  buff[0] = WriteAddr;          //先发送寄存器
  buff[1] = Data;          //再发送数据
  
  ACCEL_MAG_CS_OUT = 0;
  spi_mosi(LIS3DH_SPI, buff, buff, 2); //发送buff里数据，并采集到 buff里
  ACCEL_MAG_CS_OUT = 1;
...
status_t LIS3DH_GetAccAxesRaw(AxesRaw_t* buff) {
  i16_t value;
  u8_t *valueH = (u8_t *)(&value);
  u8_t *valueL = ((u8_t *)(&value)+1);
...
uint8 response;
uint8 lis3dh_id_read = 0;
status_t LIS3DHInit()
{
  if(LIS3DH_MEMS_I2C_ADDRESS==LIS3DH_GetWHO_AM_I());
  else
    return MEMS_ERROR;
