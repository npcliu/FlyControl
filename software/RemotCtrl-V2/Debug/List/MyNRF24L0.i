...
void SPI1_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd( ((uint32_t)0x00001000) | ((uint32_t)0x00000004), ENABLE);
   
  SPI_SSOutputCmd(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), ENABLE);
...
{
SPI1_Init();
  gpio_init(PB0, GPIO_Mode_Out_PP,n_interupt,GPIO_Speed_50MHz, 0);                               
  gpio_init(PC4, GPIO_Mode_Out_PP,n_interupt,GPIO_Speed_50MHz, 0);                               
  gpio_init(PC5, GPIO_Mode_IPU, interupt,GPIO_Speed_50MHz,1);                              
  ...
  gpio_int_cfg(PC5,((uint32_t)0x00020),EXTI_Trigger_Falling);
...
uint8 nrf_writereg(uint8 reg, uint8 dat)
{
  ...
  spi_mosi(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), buff, buff, 2); 
...
{
  spi_mosi_cmd(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), &reg , &reg, pBuf, 0, 1 , len); 
  return reg;    
}
...
{
  spi_mosi_cmd(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), &reg , &reg, 0, pBuf, 1 , len); 
...
uint8 nrf_link_check(void)
{
  ...
  reg = 0x20 + 0x10;
  spi_mosi_cmd(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), &reg, 0 , buff, 0, 1 , 5); 
  
  for(i = 0; i < 5; i++)
    buff[i] =  0;
  
  reg = 0x10;
  spi_mosi_cmd(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), &reg, 0 , 0, buff, 1 , 5); 
...
void nrf_handler(void)
{
  ...
  nrf_readreg(0x07, &nrf_state);   
  nrf_writereg(0x20 + 0x07, nrf_state);   
  
  if(nrf_state & 0x40) 
  {
    (((_32type*)(&(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00))->ODR)))->b0) = 0;
    nrf_readbuf(0x61, (uint8*)nrf_rciv, 32); 
    nrf_readreg(0x17, &nrf_fifo_sta);        
    if(nrf_fifo_sta & 0x01)                        
      rx_flag = 1;
    else if(nrf_fifo_sta & 0x02)
    {
      rx_flag = 2;
      nrf_writereg(0xE2, 0xFF);                    
    }
