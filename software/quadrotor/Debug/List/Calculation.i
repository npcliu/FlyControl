...
void CaliFilt(float *pfilted_acc,float *pfilted_gyro,float *pfilted_cps,const PACC pacc,const PGYRO pgyro,const PCOMPASS pcps,short* pacc_chip_data,short* pgyro_chip_data,short* pcps_chip_data)
{
  static short acc_filt_buff[3][5] = {0};             
  ...
  pgyro->x = pgyro_chip_data[0] - pgyro->x_offset;                      
  ...
  pfilted_acc[0] = acc_sum[0]/5;
  pfilted_acc[1] = acc_sum[1]/5;
  pfilted_acc[2] = acc_sum[2]/5;
  pfilted_gyro[0] = pgyro->x;                           
  pfilted_gyro[1] = pgyro->y;
  pfilted_gyro[2] = pgyro->z;
  pfilted_cps[0] = pcps->x;                           
...
void PWMCalc(uint8 mod)
{
  ...
  else
  {
    ...
    deriv_out = Coef*(x_d_i*_omega_error-filter_coef_state_x);
    xcq = x_b*sin(angle[0]*0.01745329252f) + x_p_i*_omega_error + deriv_out;
    filter_coef_state_x += deriv_out*5*1e-3;
    
    omega_e = y_p_o*angle_error_y;  
    _omega_error = omega_e-gyro.x;
    deriv_out = Coef*(y_d_i*_omega_error-filter_coef_state_y);
    ycq = y_b*sin(angle[1]*0.01745329252f) + y_p_i*_omega_error + deriv_out;
    filter_coef_state_y += deriv_out*5*1e-3;
  }
  dir_angle_error[0] = offset_angle[2] - angle[2];
  if(dir_angle_error[0]>200)
...
{
  static float last_speed = 0, previous_dir = 0;            
  static float _acc = 0;
  float theta = 0,fi = 0;  
  ...
  if(info->direction>-0.1 && info->direction<0.1 && info->speed>0.5 && last_speed>0.5)
    fi = previous_dir*0.01745329252f;
  else                  
  {
    fi = info->direction*0.01745329252f;
  ...
  if(ctrl[3])
  {
    angy_err = atan(-sin_theta * cos(fi) / cos(theta))/0.01745329252f;
    angx_err = -asin(sin_theta*sin_fi)/0.01745329252f;
  }
  else
  {
    float cos_fi = cos(fi);
    ...
    angy_err = acos(cos(theta)*sqrt((1-squre_sin_theta*squre_cos_fi)/(squre_sin_theta*squre_sin_theta*squre_cos_fi-2*squre_sin_theta*squre_cos_fi+1))) / 0.01745329252f;
    angx_err = asin(sin_theta*cos(fi))/0.01745329252f;
  }
}
