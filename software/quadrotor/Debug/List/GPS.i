...
unsigned char Ublox_Cfg_Rate(unsigned short measrate,unsigned char reftime)
{
  _ublox_cfg_rate cfg_ratedata={0};
  _ublox_cfg_rate *cfg_rate=&cfg_ratedata;
  if(measrate<200)
    return 1;	
  ...
  Ublox_Send_Date((unsigned char*)cfg_rate,sizeof(_ublox_cfg_rate));
  return 1;
}
void Ublox_Send_Date(unsigned char* dbuf,unsigned short len)
{
  ...
  for(j=0;j<len;j++)
  {
    ...
    ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4800))->DR=dbuf[j];  
  }	
}
