...
uint8 L3GD20WriteReg(uint8 reg, uint8 dat)
{
  ...
  (((_32type*)(&(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00))->ODR)))->b8) = 0;
  spi_mosi(((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x3C00)), buff, buff, 2); 
  (((_32type*)(&(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0C00))->ODR)))->b8) = 1;
...
void L3GD20_IO_Init()
{
  gpio_init(PB9, GPIO_Mode_IN_FLOATING, interupt,GPIO_Speed_50MHz,0);                              
  gpio_int_cfg(PB9,((uint32_t)0x00200),EXTI_Trigger_Falling);
}
...
char L3GD20_Init(uint8 L3gd20_full_scale)
{
  ...
  L3GD20_IO_Init();
  tmp=L3GD20ReadReg(0x0F);
  if(tmp==0xD4)
  {
    L3GD20WriteReg(0x20,0x3F);
    L3GD20WriteReg(0x23,L3gd20_full_scale);
    L3GD20WriteReg(0x21,0x00);
    L3GD20WriteReg(0x23,0x10);
...
void L3GD20_Read(short *pgyro)
{
  pgyro[0]=(int16_t)((L3GD20ReadReg(0x29)<<8)|
                     L3GD20ReadReg(0x28)) - pgyro[3];
  pgyro[1]=(int16_t)((L3GD20ReadReg(0x2B)<<8)|
                     L3GD20ReadReg(0x2A)) - pgyro[4];
  pgyro[2]=(int16_t)((L3GD20ReadReg(0x2D)<<8)|
                     L3GD20ReadReg(0x2C)) - pgyro[5];
}
...
char L3GD20GyroCalibration(short *PGYRO)
{
  ...
  for(i=0;i<3;i++)      
  {
    for(j=0;j<100;j++)
    {
      ...
      for(k=0;k<3;k++)
      {
        gyro_cali_sum[k] += _L3GDgyro_adc[k];              
        if(gyro_min[k]>_L3GDgyro_adc[k])
          gyro_min[k] = _L3GDgyro_adc[k];
        if(gyro_max[k]<_L3GDgyro_adc[k])
          gyro_max[k] = _L3GDgyro_adc[k];
        if(gyro_max[k] - gyro_min[k] > l3gd_gyro_cali_tolerance)        
          recali_flag = 1;
      }
      if(recali_flag)
      {
