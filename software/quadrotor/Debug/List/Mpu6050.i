...
BOOL MPUInit(void)
{
  gpio_init(PC14, GPIO_Mode_IN_FLOATING, interupt,GPIO_Speed_50MHz,0);                              
  gpio_int_cfg(PC14,((uint32_t)0x04000),EXTI_Trigger_Rising);
  
  uint8 filt_para = FLASH_ReadByte(0x0803F000 + 2); 
...
((0==acc_scale_config || 1==acc_scale_config || 2==acc_scale_config || 3==acc_scale_config) ? (void)0 : ( __aeabi_assert("0==acc_scale_config || 1==acc_scale_config || 2==acc_scale_config || 3==acc_scale_config", "E:\\Elec\\Air plane\\FlyCtrl\\CTRL_PCBV5\\User\\src\\Mpu6050.c", 33), ( __iar_EmptyStepPoint())));       
  const uint8 param[][2] = 
  {
    ...
    {0x19,        4     }, 
    {0x1A,    filt_para     }, 
    {0x1B,       gyro_scale_config<<3  }, 
    {0x1C,      acc_scale_config<<3  }, 
    {0x37,       0x12  }, 
    {0x38,        1    }, 
    {0x6A,         0x00  }, 
  };
  uint8 i;
  for(i=0; i<8; i++)
  if(false==Single_Write(0xD0, param[i][0], param[i][1]))
    return false;
  return true;
}
...
{
  uint8 accbuff[2] = {0};            
  accbuff[0] = Single_Read(0xD0,0x3C);
  accbuff[1] = Single_Read(0xD0,0x3B);
  pacc[0] = ((accbuff[1]<<8)|accbuff[0]);
  accbuff[0] = Single_Read(0xD0,0x3E);
  accbuff[1] = Single_Read(0xD0,0x3D);
  pacc[1] = ((accbuff[1]<<8)|accbuff[0]);
  accbuff[0] = Single_Read(0xD0,0x40);  
  accbuff[1] = Single_Read(0xD0,0x3F);
...
{
  uint8 gyrobuff[2] = {0};            
  gyrobuff[0] = Single_Read(0xD0,0x44);
  gyrobuff[1] = Single_Read(0xD0,0x43);
  pgyro[0] = ((gyrobuff[1]<<8)|gyrobuff[0]);
  
  gyrobuff[0] = Single_Read(0xD0,0x46);
  gyrobuff[1] = Single_Read(0xD0,0x45);
  pgyro[1] = ((gyrobuff[1]<<8)|gyrobuff[0]);
  
  gyrobuff[0] = Single_Read(0xD0,0x48);
  gyrobuff[1] = Single_Read(0xD0,0x47);
