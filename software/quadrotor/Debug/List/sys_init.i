...
void ParamInit(short *p_gyro_offset, PACC p_acc,PCOMPASS p_compass)
{
  ...
  tmp = FlashReadFloat(0x0803F800 + 8);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)             
    p_acc->kx = 1;                      
  else
    p_acc->kx = tmp;

  tmp = FlashReadFloat(0x0803F800 + 12);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_acc->ky = 1;                
  else
    p_acc->ky = tmp;

  tmp = FlashReadFloat(0x0803F800 + 16);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_acc->kz = 1;                
  else
    p_acc->kz = tmp;
  
  tmp = FlashReadFloat(0x0803F800 + 20);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_acc->x_off = 0;                
  else
    p_acc->x_off = tmp;

  tmp = FlashReadFloat(0x0803F800 + 24);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_acc->y_off = 0;                
  else
    p_acc->y_off = tmp;

  tmp = FlashReadFloat(0x0803F800 + 28);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_acc->z_off = 0;                
  else
    p_acc->z_off = tmp;
  p_compass->x_offset = FLASH_ReadHalfWord(0x0803F800 + 32);
  p_compass->y_offset = FLASH_ReadHalfWord(0x0803F800 + 34);
  p_compass->z_offset = FLASH_ReadHalfWord(0x0803F800 + 36);
  tmp = FlashReadFloat(0x0803F800 + 38);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_compass->kx = 1;                
  else
    p_compass->kx = tmp;

  tmp = FlashReadFloat(0x0803F800 + 42);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_compass->ky = 1;                
  else
    p_compass->ky = tmp;

  tmp = FlashReadFloat(0x0803F800 + 46);
  if((__c99_generic(tmp,,, __iar_Dtest, __iar_FDtest, __iar_Dtest,,,)(tmp) == 2) || -1==tmp)     
    p_compass->kz = 1;                
  else
    p_compass->kz = tmp;
...
float BatVoltageGet()
{
  ...
  if(prop_voltage<=6)         
    ...
    return 0.2249*prop_voltage*prop_voltage - 2.4531*prop_voltage + 12.999+0.2;
}
