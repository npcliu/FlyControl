...
{
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);          //使能定时器3时钟，用于定时器中断
  
  TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;	        //
  TIM_ClearITPendingBit(TIM3,TIM_IT_Update);	                //清除中断挂起位
  TIM_TimeBaseStructure.TIM_Period = nms;		        //定时器初值，（自动装载）
  TIM_TimeBaseStructure.TIM_Prescaler = 32768;		        //预分频系数
  ...
  TIM_ITConfig(TIM3,TIM_IT_Update, ENABLE);			//使能TIM3为中断源
  TIM_Cmd(TIM3, ENABLE);                                        //打开定时器3外设
}
...
{
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);          //使能定时器3时钟，用于定时器中断
  ...
  TIM_ITConfig(TIM4,TIM_IT_Update, ENABLE);			//使能TIM3为中断源
  TIM_Cmd(TIM4, ENABLE);                                        //打开定时器3外设
}
...
void TIM1_PWM_Init(uint32 freq, float duty)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1 | RCC_APB2Periph_GPIOE | RCC_APB2Periph_AFIO, ENABLE);//要想开启TIM1重映射，必须开启AFIO时钟
  
  GPIO_PinRemapConfig(GPIO_FullRemap_TIM1, ENABLE); //Timer1部分重映射  TIM3_CH2->PB5
  
  TIM_TimeBaseInitStruct.TIM_Period = TIM1_PWM_PERIOD-1; /*400Hz(2.5ms) */
  ...
  TIM_OCInitStruct.TIM_Pulse = (uint32)(TIM1_PWM_PERIOD*duty);
  TIM_OCInitStruct.TIM_OCPolarity = TIM_OCPolarity_High;
  TIM_OC1Init(TIM1, &TIM_OCInitStruct);
  TIM_OC2Init(TIM1, &TIM_OCInitStruct);
  TIM_OC3Init(TIM1, &TIM_OCInitStruct);
  
  TIM_OC1PreloadConfig(TIM1, TIM_OCPreload_Enable);     //PE9
  TIM_OC2PreloadConfig(TIM1, TIM_OCPreload_Enable);     //PE11
  TIM_OC3PreloadConfig(TIM1, TIM_OCPreload_Enable);     //PE13
...
void TIM2_PWM_Init(u32 freq ,u16 scope,int duty)
{
  ...
  float psc, arr;
  arr = scope;
  psc = 72e6/(freq*(arr+1))-1;
  //  arr = (u16)(arr+0.5);         //四舍五入
  //  psc = (u16)(psc+0.5);
  ...
  //引脚功能配置
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3; //TIM2_CH1 TIM2_CH2
...
void TIM3_PWM_Init(u32 freq ,u16 scope,int duty)
{
  ...
  //引脚功能配置
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1; //TIM2_CH1 TIM2_CH2
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;          //复用推挽输出！！
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
//  TIM_OC3Init(TIM3, &TIM_OCInitStructure);    //根据T指定的参数初始化外设TIM2 OC1
  TIM_OCInitStructure.TIM_Pulse = duty;
  TIM_OC4Init(TIM3, &TIM_OCInitStructure);  //根据T指定的参数初始化外设TIM2 OC2
  
//  TIM_OC1PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM2在CCR2上的预装载寄存器
//  TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM2在CCR2上的预装载寄存器
//  TIM_OC3PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM2在CCR2上的预装载寄存器
  TIM_OC4PreloadConfig(TIM3, TIM_OCPreload_Enable);  //使能TIM2在CCR2上的预装载寄存器
